...
namespace SubSonic.Tests.Unit.Linq.TestBases
{
    ...
    public abstract class SelectTests : LinqTestsBase
    {
        ...
        {
            CultureInfo threadNativeCulture = SetCulture();
            var result =
                _db.Customers.Where(
            ...
            RestoreCulture(threadNativeCulture);
        }

        private static void RestoreCulture(CultureInfo threadNavieCulture)
        {
            Thread.CurrentThread.CurrentCulture = threadNavieCulture;
        }

        private static CultureInfo SetCulture()
        {
            CultureInfo threadNativeCulture = Thread.CurrentThread.CurrentCulture;
            CultureInfo ukCulture = new CultureInfo("en-GB");
            Thread.CurrentThread.CurrentCulture = ukCulture;
            return threadNativeCulture;
        }
        //[Fact]
